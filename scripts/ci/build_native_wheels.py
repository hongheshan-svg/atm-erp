...
TARGETS = {
    ...
    'macos': {'x86_64': ['macosx_12_0_x86_64', 'macosx_11_0_x86_64', 'macosx_10_9_x86_64', 'macosx_10_9_universal2', 'macosx_11_0_universal2'],
...
def build(platform, output):
    ...
    for arch, tags in TARGETS[platform].items():
        ...
        args = [sys.executable, '-m', 'pip', 'download', '--only-binary=:all:', '--python-version', '311',
                '--implementation', 'cp', '--abi', 'cp311', '-r', str(root / 'backend/requirements.txt'), '-d', str(directory)]
        for tag in tags:
