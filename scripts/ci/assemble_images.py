...
if __name__ == '__main__':
    ...
    subprocess.run(
        [
            ...
            '--tag',
            REPO + ':' + args.tag,
            *(REPO + '@' + record['digest'] for record in records.values()),
        ],
        check=True,
    )
    result = subprocess.check_output(
        ['docker', 'buildx', 'imagetools', 'inspect', REPO + ':' + args.tag, '--format', '{{json .Manifest}}'],
        text=True,
    )
    digest = json.loads(result)['digest']
    (args.folder / 'manifest.json').write_text(
        json.dumps({'docker_image': REPO + '@' + digest, 'docker_archives': records}, indent=2)
    )
