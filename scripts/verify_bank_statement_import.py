...
Run inside the backend container:
    docker compose exec -T app python - < scripts/verify_bank_statement_import.py
"""
...
def main():
    ...
    assert resp1.status_code == 200, (
        f"import should succeed; got {resp1.status_code}: {resp1.data}"
    )
    assert resp1.data["header_row"] == 1, (
        f"expected header_row=1, got {resp1.data.get('header_row')}"
    )
    assert resp1.data["success_count"] == 2, (
        f"expected 2 success, got {resp1.data['success_count']}"
    )
    assert resp1.data["skipped_count"] == 1, (
        f"expected 1 skipped, got {resp1.data['skipped_count']}"
    )
    ...
    assert resp2.status_code == 200, (
        f"import should succeed; got {resp2.status_code}: {resp2.data}"
    )
    assert resp2.data["header_row"] == 3, (
        f"expected header_row=3, got {resp2.data.get('header_row')}"
    )
    ...
    assert resp3.status_code == 400, (
        f"garbage should be rejected with 400; got {resp3.status_code}"
    )
    assert "表头" in resp3.data.get("error", ""), (
        f"expected header-failure message, got {resp3.data}"
    )
    ...
    body = (
        b"".join(resp_exp.streaming_content)
        if hasattr(resp_exp, "streaming_content")
        else resp_exp.content
    )
