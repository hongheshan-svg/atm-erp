...
func (h *Handler) List(c *gin.Context) {
	...
	httpx.OK(c, httpx.Page[Supplier]{Count: total, Results: rows})
}
...
// Register 挂载路由,权限码 masterdata:supplier:*。
func (h *Handler) Register(rg *gin.RouterGroup, perm func(string) gin.HandlerFunc) {
	g := rg.Group("/masterdata/suppliers")
	g.GET("", perm("masterdata:supplier:view"), h.List)
	g.GET("/:id", perm("masterdata:supplier:view"), h.Retrieve)
	g.POST("", perm("masterdata:supplier:create"), h.Create)
	g.PUT("/:id", perm("masterdata:supplier:update"), h.Update)
	g.POST("/:id/change_status", perm("masterdata:supplier:update"), h.ChangeStatus)
	g.DELETE("/:id", perm("masterdata:supplier:delete"), h.Delete)
}
...
func statusFor(err error) int {
	...
	case errors.Is(err, ErrCodeExists), errors.Is(err, ErrInvalidStatus), errors.Is(err, ErrInvalidSettlement):
