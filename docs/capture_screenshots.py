...
# 配置
BASE_URL = os.getenv("ERP_BASE_URL", "http://localhost/erp").rstrip("/")
...
async def capture_screenshots():
    """截取所有页面截图"""
    if not PASSWORD:
        raise RuntimeError("请通过 ERP_PASSWORD 环境变量提供截图账号密码")
    ...
    async with async_playwright() as p:
        ...
        await page.screenshot(
            path=os.path.join(SCREENSHOT_DIR, "login.png"), full_page=False
        )
        print("   ✅ login.png")
        ...
        try:
            # 填写用户名
            await page.fill(
                'input[placeholder*="用户名"], input[type="text"]', USERNAME
            )
            # 填写密码
            await page.fill(
                'input[placeholder*="密码"], input[type="password"]', PASSWORD
            )
            # 点击登录按钮
        ...
        for filename, path, wait_time, special in PAGES_TO_CAPTURE:
            ...
            try:
                print(f"📸 截取 {filename}...")
                await page.goto(
                    f"{BASE_URL}{path}", wait_until="networkidle", timeout=30000
                )
...
async def capture_form_screenshots():
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            # 点击新建按钮
            await page.click(
                'button:has-text("新建"), button:has-text("新增"), button:has-text("添加")'
            )
            await asyncio.sleep(1)
            await page.screenshot(
                path=os.path.join(SCREENSHOT_DIR, "customer_form.png")
            )
