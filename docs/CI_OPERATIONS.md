...
- 前端版本字段单独递增与依赖更新区别处理：纯版本变更只执行前端检查；前端或 Python 依赖变化按共享路径覆盖全部业务模块，运行时 Python 依赖还会运行安装器与 OTA。数据库迁移除全部业务模块外还会运行 OTA 升级演练（升级先备份再前向迁移）。
...
| suite=release | 发版验证：比较前一正式 tag，后端、前端单测与运维脚本测试全部运行；浏览器按改动模块及关联页面（不含完整业务链），OTA 与安装器按相关改动 |
...
4. 没有匹配记录时 Release 以 suite=release、双视口运行发版验证：全部后端阶段（平台、业务、并发）、全部前端单测与运维脚本测试全部运行；浏览器只跑前一正式 tag 以来改动的模块及关联页面用例（共享核心改动和未登记路径覆盖全部业务模块），不含完整业务链；OTA 在升级代码、数据库迁移、Docker/部署或运行时依赖变化时运行，安装器在安装与打包脚本、Docker/部署或运行时依赖变化时运行。纯业务或文档发布不启动 OTA 与安装器。force_validation 强制重跑。不修改已存在 tag 来绕过失败。完整业务链只在明确要求时手动运行 suite=full。
