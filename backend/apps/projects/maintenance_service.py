...
def check_equipment_maintenance():
    ...
    results = {'warranty_expiring': [], 'maintenance_due': [], 'calibration_due': []}

    # 1. 检查保修到期
    for days in reminder_days:
        check_date = today + timedelta(days=days)
        expiring_equipment = Equipment.objects.filter(warranty_end_date=check_date, is_deleted=False)

        for eq in expiring_equipment:
            results['warranty_expiring'].append({'equipment': eq.name, 'code': eq.equipment_no, 'days_remaining': days})
...
def _get_equipment_notify_users(equipment):
    ...
    # 设备管理员（按角色查找）
    equipment_managers = User.objects.filter(groups__name__in=['equipment_manager', 'admin'], is_active=True)
...
def generate_maintenance_report():
    ...
    completed_maintenance = MaintenanceSchedule.objects.filter(
        status='COMPLETED', actual_date__gte=last_month_start, actual_date__lte=last_month_end
    ).count()

    # 统计超期未完成的维保
    overdue_maintenance = MaintenanceSchedule.objects.filter(status='PENDING', next_maintenance_date__lt=today).count()

    # 统计设备状态
    equipment_stats = Equipment.objects.filter(is_deleted=False).values('status').annotate(count=models.Count('id'))

    # 统计保修状态
    in_warranty = Equipment.objects.filter(warranty_end_date__gte=today, is_deleted=False).count()

    out_of_warranty = Equipment.objects.filter(warranty_end_date__lt=today, is_deleted=False).count()
