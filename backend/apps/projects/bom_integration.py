...
class BOMPurchaseService:
    ...
    def create_purchase_request_from_bom(
        cls, project_id: int, bom_item_ids: List[int], user, title: str = None, notes: str = ''
        ...
        bom_items = ProjectBOM.objects.filter(
            id__in=bom_item_ids, project_id=project_id, is_deleted=False
        ...
        with transaction.atomic():
            ...
            for bom in bom_items:
                ...
                # 获取预估单价
                estimated_price = (
                    bom.estimated_cost or bom.item.purchase_price or bom.item.standard_cost or Decimal('0')
                )
...
class BOMProductionService:
    ...
    def create_production_plan_from_bom(
        ...
        project = Project.objects.get(id=project_id)
        bom_items = (
            ProjectBOM.objects.filter(id__in=bom_item_ids, project_id=project_id, is_deleted=False)
            .select_related('item', 'work_center', 'process')
            .order_by('assembly_sequence')
        )
        ...
        with transaction.atomic():
            ...
            for wc_id, boms in work_center_boms.items():
                ...
                process = ProductionProcess.objects.create(
                    ...
                    description='\n'.join([f'- {b.item.sku}: {b.item.name} x {b.planned_qty}' for b in boms]),
                    created_by=user,
                )
    ...
    def get_bom_material_requirements(cls, project_id: int) -> Dict:
        ...
        bom_items = ProjectBOM.objects.filter(project_id=project_id, is_deleted=False).select_related('item')
...
class BOMPurchaseRequestSerializer(serializers.Serializer):
    ...
    bom_item_ids = serializers.ListField(child=serializers.IntegerField(), required=True, min_length=1)
...
class BOMIntegrationViewSet(viewsets.ViewSet):
    ...
    def get_purchasable_items(self, request, project_id=None):
        ...
        items = BOMPurchaseService.get_purchasable_bom_items(int(project_id), include_ordered=include_ordered)
