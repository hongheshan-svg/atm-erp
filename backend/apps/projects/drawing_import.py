...
class DrawingBOMLinkService:
    ...
    def match_drawing_to_bom(cls, drawing_no: str, project_id: int) -> Dict:
        ...
        # 1. 精确匹配 - drawing_no字段
        exact_match = (
            ProjectBOM.objects.filter(project_id=project_id, drawing_no__iexact=drawing_no_clean, is_deleted=False)
            .select_related('item')
            .first()
        )
        ...
        # 2. 精确匹配 - item.sku
        sku_match = (
            ProjectBOM.objects.filter(project_id=project_id, item__sku__iexact=drawing_no_clean, is_deleted=False)
            .select_related('item')
            .first()
        )

        if sku_match:
            return {'matched': True, 'bom_item': sku_match, 'match_type': 'EXACT_SKU', 'score': 1.0, 'candidates': []}

        # 3. 精确匹配 - item_code
        code_match = (
            ProjectBOM.objects.filter(project_id=project_id, item_code__iexact=drawing_no_clean, is_deleted=False)
            .select_related('item')
            .first()
        )
        ...
        # 5. 模糊匹配
        all_bom_items = ProjectBOM.objects.filter(project_id=project_id, is_deleted=False).select_related('item')
        ...
        return {'matched': False, 'bom_item': None, 'match_type': '', 'score': 0, 'candidates': candidates}
    ...
    def batch_link_drawings(cls, project_id: int, auto_create_missing: bool = False) -> Dict:
        ...
        return {'linked': linked, 'not_found': not_found, 'already_linked': already_linked, 'details': details}
            ...
            try:
                ...
                if parsed['file_type'] == 'OTHER':
                    ...
                    if ext.lower() not in cls.SUPPORTED_EXTENSIONS:
                        skipped += 1
                        details.append(
                            {'filename': filename, 'status': 'SKIPPED', 'reason': f'不支持的文件类型: {ext}'}
                        )
                        continue

                # 检查是否已存在
                existing = Drawing.objects.filter(
                    project=project, drawing_no=parsed['drawing_no'], version=parsed['version'], is_deleted=False
                ).first()

                if existing and not overwrite:
                    skipped += 1
                    details.append(
                        {'filename': filename, 'status': 'SKIPPED', 'reason': '图纸已存在', 'existing_id': existing.id}
                    )
                    continue
                ...
                # 自动关联BOM
                if auto_link:
                    link_result = DrawingBOMLinkService.match_drawing_to_bom(drawing.drawing_no, project_id)
...
class DrawingImportViewSet(viewsets.ViewSet):
    ...
    def batch_import(self, request):
        ...
        if zip_file:
            result = BatchDrawingImportService.import_from_zip(zip_file, int(project_id), request.user, options)
        elif files:
            result = BatchDrawingImportService.import_files(files, int(project_id), request.user, options)
        else:
    ...
    def supported_formats(self, request):
        ...
        return Response({'formats': formats, 'max_file_size': '100MB', 'max_files': 100})
