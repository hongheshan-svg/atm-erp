...
class AcceptanceViewSet(PermissionMixin, SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    def report(self, request, pk=None):
        """生成验收报告数据，download=pdf 时返回可下载的 PDF。"""
        acceptance = self.get_object()
        ...
        if request.query_params.get('download') == 'pdf':
            from .acceptance_report import build_acceptance_report_pdf

            pdf_content = build_acceptance_report_pdf(report_data)
            response = HttpResponse(pdf_content, content_type='application/pdf')
            report_no = acceptance.acceptance_no.replace('"', '')
            response['Content-Disposition'] = f'attachment; filename="acceptance_report_{report_no}.pdf"'
            return response
