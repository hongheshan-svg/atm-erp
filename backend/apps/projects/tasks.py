...
def check_project_task_reminders():
    ...
    overdue_tasks = ProjectTask.objects.filter(
        end_date__lt=today, status__in=['TODO', 'IN_PROGRESS'], is_deleted=False
    ).select_related('project', 'assignee')

    # Find tasks ending within 3 days
    upcoming_tasks = ProjectTask.objects.filter(
        end_date__gte=today, end_date__lte=warning_date, status__in=['TODO', 'IN_PROGRESS'], is_deleted=False
    ).select_related('project', 'assignee')
    ...
    in_progress_tasks = ProjectTask.objects.filter(
        status='IN_PROGRESS', start_date__isnull=False, end_date__isnull=False, is_deleted=False
    ).select_related('project', 'assignee')
    ...
    pm_recipients = User.objects.filter(
        is_active=True, is_deleted=False, role__code__in=['PROJECT_MANAGER', 'ADMIN']
...
"""
    ...
    return (
        f'Sent task reminders: {len(overdue_items)} overdue, {len(upcoming_items)} upcoming, {len(behind_items)} behind'
    )
...
def check_project_deadline_reminders():
    ...
    recipients = (
        ...
        .filter(role__code__in=['PROJECT_MANAGER', 'ADMIN'])
        .values_list('id', flat=True)
    )
...
def check_aftersales_reminders():
    ...
    unassigned_urgent = AfterSalesOrder.objects.filter(
        assigned_to__isnull=True, priority__in=['HIGH', 'URGENT'], status='PENDING', is_deleted=False
    ...
    recipients = User.objects.filter(
        is_active=True, is_deleted=False, role__code__in=['AFTER_SALES', 'SERVICE', 'ADMIN']
    ).values_list('id', flat=True)
