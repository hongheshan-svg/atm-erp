...
class WorkCenter(BaseModel):
    ...
    # 产能参数
    capacity_per_day = models.DecimalField(max_digits=10, decimal_places=2, default=8, verbose_name='日产能(小时)')
    efficiency = models.DecimalField(max_digits=5, decimal_places=2, default=100, verbose_name='效率(%)')

    # 关联设备
    equipment = models.ManyToManyField(
        'projects.Equipment', blank=True, related_name='work_centers', verbose_name='设备'
    )
...
class ProductionSchedule(BaseModel):
    ...
    project = models.ForeignKey(
        'projects.Project', on_delete=models.CASCADE, related_name='production_schedules', verbose_name='项目'
    )
    work_center = models.ForeignKey(
        WorkCenter, on_delete=models.SET_NULL, null=True, blank=True, related_name='schedules', verbose_name='工作中心'
    )
...
class ScheduleTask(BaseModel):
    ...
    schedule = models.ForeignKey(
        ProductionSchedule, on_delete=models.CASCADE, related_name='tasks', verbose_name='排程', null=True, blank=True
    )
...
class KanbanView(APIView):
    ...
    def get(self, request):
        ...
        # 生产概况
        production_stats = {
            'total_schedules': ProductionSchedule.objects.filter(planned_start__date=today, is_deleted=False).count(),
            'in_progress': ProductionSchedule.objects.filter(status='IN_PROGRESS', is_deleted=False).count(),
            'completed_today': ProductionSchedule.objects.filter(
                actual_end__date=today, status='COMPLETED', is_deleted=False
            ).count(),
        ...
        # 项目进度
        projects = Project.objects.filter(status='IN_PROGRESS', is_deleted=False).order_by('-created_at')[:10]
        ...
        for wc in work_centers:
            ...
            scheduled_hours = (
                ProductionSchedule.objects.filter(
                    work_center=wc,
        ...
        # 今日排程列表
        today_schedules = (
            ProductionSchedule.objects.filter(planned_start__date=today, is_deleted=False)
            .select_related('project', 'work_center', 'assignee')
            .order_by('priority', 'planned_start')[:20]
        )
...
class GanttView(APIView):
    ...
    def get(self, request):
        ...
        qs = ProductionSchedule.objects.filter(
            is_deleted=False, status__in=['CONFIRMED', 'RELEASED', 'IN_PROGRESS']
        ).select_related('project', 'work_center', 'assignee')
...
class ProductionScheduleViewSet(SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    def by_date(self, request):
        ...
        schedules = self.get_queryset().filter(planned_start__date=query_date).order_by('priority', 'planned_start')
