...
class ProductionKanbanView(APIView):
    ...
    def get(self, request):
        ...
        # 今日生产概览
        today_orders = ScheduleOrder.objects.filter(planned_start__date=today, is_deleted=False)
        ...
        if overview['total_orders'] > 0:
            overview['completion_rate'] = round(overview['completed'] / overview['total_orders'] * 100, 1)
        ...
        for wc in work_centers:
            current_order = ScheduleOrder.objects.filter(work_center=wc, status='IN_PROGRESS', is_deleted=False).first()

            today_completed = ScheduleOrder.objects.filter(
                work_center=wc, status='COMPLETED', actual_end__date=today, is_deleted=False
            ).count()

            today_planned = ScheduleOrder.objects.filter(
                work_center=wc, planned_start__date=today, is_deleted=False
            ).count()
        ...
        # 进行中的工单
        active_orders = (
            ScheduleOrder.objects.filter(status='IN_PROGRESS', is_deleted=False)
            .select_related('work_center', 'item')
        ...
        if quality_data['total'] > 0:
            quality_data['pass_rate'] = round(quality_data['passed'] / quality_data['total'] * 100, 1)
    ...
    def get(self, request, work_center_id):
        ...
        if current_order:
            current_task = {
                ...
                'progress': int(current_order.completed_qty / current_order.quantity * 100)
        ...
        # 待处理队列
        queue = ScheduleOrder.objects.filter(work_center=wc, status='SCHEDULED', is_deleted=False).order_by(
        ...
        return Response(
            {
                'work_center': {'id': wc.id, 'name': wc.name, 'efficiency': float(wc.efficiency)},
...
class ProductionTrendView(APIView):
    ...
    def get(self, request):
        ...
        while current <= end_date:
            day_orders = ScheduleOrder.objects.filter(actual_end__date=current, status='COMPLETED', is_deleted=False)
        ...
        return Response({'period': {'start': start_date.isoformat(), 'end': end_date.isoformat()}, 'trend': trend})
...
class AndonAlertView(APIView):
    ...
    def get(self, request):
        ...
        # 1. 生产延期预警
        delayed = ScheduleOrder.objects.filter(status='IN_PROGRESS', planned_end__lt=now, is_deleted=False)
        ...
        # 2. 设备故障预警
        try:
            faulty_equipment = Equipment.objects.filter(status='FAULT', is_deleted=False)
