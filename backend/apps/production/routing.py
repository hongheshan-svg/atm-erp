...
class WorkStation(BaseModel):
    ...
    station_type = models.CharField(max_length=20, choices=STATION_TYPE_CHOICES, default='OTHER', verbose_name='工位类型')
...
class RoutingTemplate(BaseModel):
    ...
    def calculate_totals(self):
        ...
        totals = self.operations.filter(is_deleted=False).aggregate(
            total_standard=Sum('standard_hours'),
            total_setup=Sum('setup_hours')
        )
...
class RoutingOperation(BaseModel):
    ...
    operation_type = models.CharField(max_length=20, choices=OPERATION_TYPE_CHOICES, default='OTHER', verbose_name='工序类型')
...
class OperationMaterial(BaseModel):
    ...
    item = models.ForeignKey(
        ...
        related_name='operation_consumptions',
        verbose_name='物料'
    )
...
class ProjectRoutingOperation(BaseModel):
    ...
    sequence = models.IntegerField(verbose_name='工序号')
    operation_name = models.CharField(max_length=200, verbose_name='工序名称')
    operation_type = models.CharField(max_length=20, choices=RoutingOperation.OPERATION_TYPE_CHOICES, default='OTHER', verbose_name='工序类型')
...
class RoutingTemplateListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = RoutingTemplate
        fields = [
            'id', 'code', 'name', 'product_category', 'product_category_name',
            'version', 'is_current', 'total_standard_hours', 'total_setup_hours',
            'status', 'status_display', 'operation_count', 'is_active', 'created_at'
        ]
...
class WorkStationViewSet(SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    def capacity_summary(self, request):
        ...
        by_type = stations.values('station_type').annotate(
            ...
            total_capacity=Sum('standard_capacity'),
            avg_uph=Avg('uph')
        )
...
class ProjectRoutingOperationViewSet(SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    def complete(self, request, pk=None):
        ...
        routing.total_actual_hours = routing.operations.filter(
            is_deleted=False
