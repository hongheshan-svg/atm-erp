...
class AnalyticsViewSet(PermissionMixin, viewsets.ViewSet):
    ...
    def management_dashboard(self, request):
        ...
        ).aggregate(total=Sum('amount_due') - Sum('amount_paid'), count=Count('id'))
        ...
        active_projects = Project.objects.filter(
            status__in=['IN_PROGRESS', 'ACTIVE', 'PLANNING'], is_deleted=False
        ).aggregate(count=Count('id'), budget_total=Sum('budget_total'))
        ...
        upcoming_payables_list = (
            AccountPayable.objects.filter(
                status__in=['PENDING', 'PARTIAL', 'OVERDUE'], due_date__lte=today + timedelta(days=7), is_deleted=False
            )
        ...
        task_stats = {
            row['project_id']: row
            for row in ProjectTask.objects.filter(project__in=active_projects_list, is_deleted=False)
            .values('project_id')
            .annotate(
                total=Count('id'),
