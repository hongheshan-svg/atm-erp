...
from apps.finance.models import Expense, AccountReceivable, AccountPayable
...
class DashboardKPIService:
    ...
    def get_project_kpis():
        ...
        task_completion = tasks.aggregate(
            total=Count('id'),
    ...
    def get_inventory_kpis():
        ...
        recent_moves = StockMove.objects.filter(
            move_date__gte=datetime.now() - timedelta(days=7)
        ).count()
...
class CashFlowForecastService:
    ...
    def forecast_next_30_days():
        ...
        expected_inflows = AccountReceivable.objects.filter(
            due_date__range=[today, forecast_end],
            status__in=['PENDING', 'PARTIAL']
        ).aggregate(
        ...
        # Expected payables
        expected_outflows = AccountPayable.objects.filter(
            due_date__range=[today, forecast_end],
            status__in=['PENDING', 'PARTIAL']
        ).aggregate(
            total=Sum(F('amount_due') - F('amount_paid'))
        ...
        avg_monthly_expense = Expense.objects.filter(
            expense_date__gte=today - timedelta(days=90),
            status='APPROVED'
...
class InventoryAnalyticsService:
    ...
    def calculate_turnover_rate(days=30):
        ...
        avg_inventory = Stock.objects.aggregate(
            total_value=Sum(F('qty_on_hand') * F('weighted_avg_cost'))
        )['total_value'] or 0
    ...
    def get_slow_moving_items(days=90):
        ...
        moved_item_ids = StockMove.objects.filter(
            move_date__gte=start_date
        ).values_list('item_id', flat=True).distinct()
        
        slow_moving = Stock.objects.exclude(
            item_id__in=moved_item_ids
            ...
            value=F('qty_on_hand') * F('weighted_avg_cost')
        )
