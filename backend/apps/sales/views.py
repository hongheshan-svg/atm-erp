...
class SalesQuotationViewSet(
    ...
    def submit(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {'error': f'审批服务暂时不可用，请稍后重试: {e}'}, status=status.HTTP_503_SERVICE_UNAVAILABLE
            )
    ...
    def convert_to_so(self, request, pk=None):
        ...
        if quotation.status not in ['APPROVED', 'SENT']:
            return Response({'error': '只能将已审批/已发送的报价单转换为订单'}, status=status.HTTP_400_BAD_REQUEST)
...
class SalesQuotationLineViewSet(PermissionMixin, SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    permission_module = 'sales'
    permission_resource = 'quotation_line'
    """
...
class SalesOrderViewSet(
    ...
    def _do_confirm(self, so, request):
        ...
        with transaction.atomic():
            ...
            schedules = create_sales_order_receivables(so, request.user, request.data.get('due_date', so.delivery_date))
    ...
    def cancel(self, request, pk=None):
        ...
        active_deliveries = (
            DeliveryOrder.objects.filter(so=so, is_deleted=False).exclude(status__in=['DRAFT', 'REJECTED']).exists()
        )
        if active_deliveries:
            return Response(
                {'error': '该订单存在已提交/在途的发货单，请先拒绝或退货后再取消'}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        with transaction.atomic():
            ...
            DeliveryOrder.objects.filter(so=so, is_deleted=False, status__in=['DRAFT', 'REJECTED']).update(
...
class DeliveryOrderViewSet(
    ...
    def create_invoice(self, request, pk=None):
        ...
        if invoice is None:
            return Response({'error': '发货单无有效明细，无法开票'}, status=status.HTTP_400_BAD_REQUEST)
        return Response(
            {
        ...
        with transaction.atomic():
            if delivery.status in stocked_out_statuses:
                ...
                if so.status in ['PARTIAL', 'COMPLETED']:
                    any_delivered = any((sl.delivered_qty or 0) > 0 for sl in so.lines.filter(is_deleted=False))
...
class DeliveryOrderLineViewSet(PermissionMixin, SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    permission_module = 'sales'
    permission_resource = 'delivery_line'
    """
