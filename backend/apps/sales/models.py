...
class SalesQuotation(BaseModel):
    ...
    customer = models.ForeignKey(
        'masterdata.Customer', on_delete=models.PROTECT, related_name='quotations', verbose_name='客户'
    )
...
class SalesQuotationLine(BaseModel):
    ...
    quotation = models.ForeignKey(
        SalesQuotation, on_delete=models.CASCADE, related_name='lines', verbose_name='销售报价'
    )
    # 物料关联（可选，非标产品可不选）
...
class SalesOrder(BaseModel):
    ...
    customer = models.ForeignKey(
        'masterdata.Customer', on_delete=models.PROTECT, related_name='sales_orders', verbose_name='客户'
    )
    project = models.ForeignKey(
        'projects.Project',
        on_delete=models.PROTECT,
    ...
    payment_terms = models.CharField(
        max_length=20, choices=PAYMENT_TERMS_CHOICES, default='M_30_30_30_10', verbose_name='付款条款'
    )
...
class SalesOrderLine(BaseModel):
    ...
    so = models.ForeignKey(SalesOrder, on_delete=models.CASCADE, related_name='lines', verbose_name='销售订单')
    # 物料关联（可选，非标产品可不选）
    item = models.ForeignKey(
        'masterdata.Item', on_delete=models.PROTECT, related_name='so_lines', verbose_name='物料', null=True, blank=True
    )
    # 手动填写的产品信息（非标定制用）
    custom_name = models.CharField(max_length=200, blank=True, verbose_name='产品名称')
    custom_spec = models.CharField(max_length=200, blank=True, verbose_name='规格型号')
...
class DeliveryOrder(BaseModel):
    ...
    so = models.ForeignKey(SalesOrder, on_delete=models.PROTECT, related_name='deliveries', verbose_name='销售订单')
    warehouse = models.ForeignKey(
        'masterdata.Warehouse', on_delete=models.PROTECT, related_name='deliveries', verbose_name='发货仓库'
    )
    ...
    packaging_type = models.CharField(
        max_length=20, choices=PACKAGING_CHOICES, default='STANDARD', verbose_name='包装方式'
    )
    ...
    insurance_amount = models.DecimalField(
        max_digits=15, decimal_places=2, null=True, blank=True, verbose_name='保险金额'
    )
    ...
    logistics_cost = models.DecimalField(
        max_digits=15, decimal_places=2, null=True, blank=True, verbose_name='物流费用'
    )

    # 送货单签收
    signed_receipt = models.FileField(upload_to='delivery_receipts/', null=True, blank=True, verbose_name='签收单据')
...
class DeliveryOrderLine(BaseModel):
    ...
    delivery = models.ForeignKey(DeliveryOrder, on_delete=models.CASCADE, related_name='lines', verbose_name='发货单')
    so_line = models.ForeignKey(
        SalesOrderLine, on_delete=models.PROTECT, related_name='delivery_lines', verbose_name='销售订单明细'
    )
    item = models.ForeignKey(
        'masterdata.Item',
        on_delete=models.PROTECT,
...
class SalesContract(BaseModel):
    ...
    so = models.ForeignKey(SalesOrder, on_delete=models.PROTECT, related_name='contracts', verbose_name='销售订单')
    customer = models.ForeignKey(
        'masterdata.Customer', on_delete=models.PROTECT, related_name='sales_contracts', verbose_name='客户'
    )
    project = models.ForeignKey(
        'projects.Project',
        on_delete=models.SET_NULL,
...
# Import CRM models
from .crm_models import Lead, LeadSource, Opportunity, OpportunityActivity, SalesForecast  # noqa: E402, F401
