...
class ItemCategory(BaseModel):
    ...
    parent = models.ForeignKey(
        'self', on_delete=models.SET_NULL, null=True, blank=True, related_name='children', verbose_name='上级类别'
    )
...
class Item(BaseModel):
    ...
    category = models.ForeignKey(
        ItemCategory, on_delete=models.SET_NULL, null=True, blank=True, related_name='items', verbose_name='物料类别'
    )
    item_type = models.CharField(max_length=20, choices=ITEM_TYPE_CHOICES, default='MATERIAL', verbose_name='物料类型')
    ...
    abc_class = models.CharField(max_length=1, choices=ABC_CLASS_CHOICES, default='C', verbose_name='ABC分类')
    ...
    material = models.CharField(
        max_length=100, blank=True, verbose_name='材质', help_text='如：45#钢、SUS304、6061铝合金等'
    )
    surface_treatment = models.CharField(
        max_length=100, blank=True, verbose_name='表面处理', help_text='如：镀锌、发黑、阳极氧化、喷涂等'
    )
    heat_treatment = models.CharField(
        max_length=100, blank=True, verbose_name='热处理', help_text='如：调质HRC28-32、渗碳淬火等'
    )
    ...
    inspection_type = models.CharField(
        max_length=20, choices=INSPECTION_TYPE_CHOICES, default='INCOMING', verbose_name='检验方式'
    )
    inspection_standard = models.TextField(blank=True, verbose_name='检验标准')

    # 是否关键件
    is_critical = models.BooleanField(
        default=False, verbose_name='关键件', help_text='影响产品关键性能或交期的核心部件'
    )
    is_long_lead = models.BooleanField(default=False, verbose_name='长周期件', help_text='采购周期超过项目标准周期')

    # 替代品信息
    can_substitute = models.BooleanField(default=False, verbose_name='允许替代')
    alternate_items = models.ManyToManyField(
        'self', blank=True, symmetrical=False, related_name='alternate_for', verbose_name='可替代物料'
    )

    unit = models.CharField(max_length=20, choices=UNIT_CHOICES, default='PCS', verbose_name='单位')

    # ==================== 价格和税率 ====================
    standard_cost = models.DecimalField(max_digits=15, decimal_places=2, default=0, verbose_name='标准成本')
    purchase_price = models.DecimalField(max_digits=15, decimal_places=2, default=0, verbose_name='采购单价')
    sale_price = models.DecimalField(max_digits=15, decimal_places=2, default=0, verbose_name='销售单价')
    last_purchase_price = models.DecimalField(max_digits=15, decimal_places=2, default=0, verbose_name='最近采购价')
    ...
    # ==================== 库存相关 ====================
    min_stock = models.DecimalField(max_digits=15, decimal_places=2, default=0, verbose_name='最小库存')
    ...
    reorder_point = models.DecimalField(max_digits=15, decimal_places=2, default=0, verbose_name='再订货点')
    economic_order_qty = models.DecimalField(max_digits=15, decimal_places=2, default=0, verbose_name='经济订货批量')
...
class Supplier(BaseModel):
    ...
    settlement_method = models.CharField(
        max_length=20, choices=SETTLEMENT_METHOD_CHOICES, blank=True, verbose_name='结款方式'
    )
...
class WarehouseLocation(BaseModel):
    ...
    parent = models.ForeignKey(
        'self', on_delete=models.CASCADE, null=True, blank=True, related_name='children', verbose_name='上级库位'
    )
    code = models.CharField(max_length=50, verbose_name='库位编码')
    name = models.CharField(max_length=100, verbose_name='库位名称')
    location_type = models.CharField(
        max_length=20, choices=LOCATION_TYPE_CHOICES, default='BIN', verbose_name='库位类型'
    )
    ...
    max_weight = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name='最大承重(kg)'
    )
    max_volume = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name='最大容积(m³)'
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.parent:
            self.full_path = (
                f'{self.parent.full_path}-{self.code}' if self.parent.full_path else f'{self.parent.code}-{self.code}'
            )
        else:
