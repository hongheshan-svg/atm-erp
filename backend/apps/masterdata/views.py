...
class ItemViewSet(PermissionMixin, SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    def generate_code(self, request):
        ...
        if not level1_code or not level2_code:
            return Response({'error': '请提供一级代码和二级代码'}, status=status.HTTP_400_BAD_REQUEST)

        try:
            code = ItemCodeGenerator.generate_code(level1_code, level2_code)
            code_info = ItemCodeGenerator.parse_code(code)

            return Response({'code': code, 'info': code_info})
    ...
    def import_excel(self, request):
        ...
        try:
            ...
            if not name_col:
                return Response({'error': 'Excel文件必须包含"物料名称"列'}, status=status.HTTP_400_BAD_REQUEST)
                ...
                try:
                    ...
                    if not sku:
                        ...
                        if existing_item:
                            ...
                            if not level1_code:
                                error_rows.append(
                                    {'row': row_num, 'error': f'无效的"有图/无图": {level1_raw}，应为"有图"或"无图"'}
                                )
                                continue
    ...
    def export_excel(self, request):
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            format_map = {'required': required_format, 'conditional': conditional_format, 'optional': optional_format}
    ...
    def generate_barcode(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {'error': f'Barcode generation failed: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def generate_qrcode(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {'error': f'QR code generation failed: {str(e)}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class CustomerViewSet(PermissionMixin, SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    def download_template(self, request):
        ...
        # 与列表页面显示的列对应
        headers = ['客户编码', '客户名称', '联系人', '电话', '税号', '开户银行', '地址', '状态']
...
class SupplierViewSet(PermissionMixin, SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    def download_template(self, request):
        ...
        # 与列表页面显示的列对应
        headers = ['供应商编码', '供应商名称', '联系人', '电话', '税号', '开户银行', '地址', '状态']
...
class WarehouseViewSet(PermissionMixin, SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    def location_tree(self, request, pk=None):
        ...
        warehouse = self.get_object()
        root_locations = warehouse.locations.filter(parent__isnull=True, is_deleted=False).order_by(
            'sort_order', 'code'
        )
...
class WarehouseLocationViewSet(PermissionMixin, SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    def tree(self, request):
        ...
        if warehouse_id:
            root_locations = (
                self.get_queryset()
                .filter(warehouse_id=warehouse_id, parent__isnull=True, is_deleted=False)
    ...
    def bulk_create(self, request):
        ...
        for loc_data in locations_data:
            try:
                ...
                errors.append({'code': loc_data.get('code'), 'error': str(e)})

        return Response({'created': len(created), 'errors': errors, 'locations': created})
