...
class ProjectProfitabilityReportView(APIView):
    ...
    def get(self, request):
        ...
        for project in queryset.select_related('sales_order', 'customer'):
            ...
            if (
                project.sales_order
                and hasattr(project.sales_order, 'total_with_tax')
                and project.sales_order.total_with_tax
            ):
            ...
            # 实际成本
            costs = ProjectCostRecord.objects.filter(project=project, is_deleted=False).aggregate(total=Sum('amount'))
        ...
        for p in projects_data:
            ...
            if customer not in by_customer:
                by_customer[customer] = {'contract_amount': 0, 'actual_cost': 0, 'gross_profit': 0, 'project_count': 0}
        ...
        return Response(
            {
                ...
                'by_customer': [
                    {'customer': k, **v}
                    for k, v in sorted(by_customer.items(), key=lambda x: x[1]['gross_profit'], reverse=True)
                ],
            }
        )
    ...
    def get(self, request):
        ...
        if not equipment_id:
            ...
            for eq in equipments[:50]:  # 限制数量
                # 服务次数
                service_count = ServiceOrder.objects.filter(equipment=eq, is_deleted=False).count()

                # 备件消耗
                spare_cost = (
                    SparePartConsumption.objects.filter(equipment=eq, is_deleted=False).aggregate(
        ...
        # 服务历史
        services = (
            ServiceOrder.objects.filter(equipment=equipment, is_deleted=False)
            .values('service_type')
            .annotate(count=Count('id'), total_cost=Sum('actual_cost'))
        )

        # 备件消耗趋势
        spare_trend = (
            SparePartConsumption.objects.filter(equipment=equipment, is_deleted=False)
        ...
        alarms = (
            EquipmentAlarm.objects.filter(equipment=equipment, is_deleted=False)
            .values('severity')
...
class CapacityUtilizationReportView(APIView):
    ...
    def get(self, request):
        ...
        for wc in WorkCenter.objects.filter(is_deleted=False):
            # 计划工时
            planned = (
                ScheduleOrder.objects.filter(
                    work_center=wc, planned_start__lte=end, planned_end__gte=start, is_deleted=False
...
class ProjectDeliveryReportView(APIView):
    ...
    def get(self, request):
        ...
        projects = Project.objects.filter(created_at__year=year, is_deleted=False)
...
class OutsourceAnalysisReportView(APIView):
    ...
    def get(self, request):
        ...
        orders = OutsourceOrder.objects.filter(order_date__year=year, is_deleted=False)
        ...
        capability_dist = capabilities.values('process_type').annotate(
            supplier_count=Count('supplier', distinct=True), avg_rating=Avg('overall_rating')
        )
