...
def project_cost_detail(request):
    ...
    if not project_id:
        return Response({'error': '请提供project_id参数'}, status=status.HTTP_400_BAD_REQUEST)
...
def refresh_project_cache(request):
    ...
    return Response({'message': '缓存已刷新', 'data': result})
...
def dashboard_summary(request):
    ...
    low_stock_count = Stock.objects.filter(qty_on_hand__lt=DbF('item__min_stock'), item__min_stock__gt=0).count()

    total_stock_value = Stock.objects.aggregate(total=Sum(DbF('qty_on_hand') * DbF('weighted_avg_cost')))['total'] or 0

    # Get top profitable projects
    df_projects = CostCalculationService.calculate_all_projects_profit()
    if not df_projects.empty:
        top_projects = df_projects.nlargest(5, 'profit')[['code', 'name', 'profit', 'margin_percent']].to_dict(
            'records'
        )
    else:
        top_projects = []
    ...
    return Response(
        {
            ...
            'inventory': {'low_stock_items': low_stock_count, 'total_value': float(total_stock_value)},
...
def aging_report(request):
    ...
    return Response({'results': results, 'summary': summary, 'type': report_type})
