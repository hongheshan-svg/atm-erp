...
class DashboardWidgetViewSet(PermissionMixin, viewsets.ModelViewSet):
    ...
    def widget_types(self, request):
        """获取组件类型选项"""
        return Response([{'value': choice[0], 'label': choice[1]} for choice in DashboardWidget.WIDGET_TYPES])

    @action(detail=False, methods=['get'])
    def data_sources(self, request):
        """获取数据源选项"""
        return Response([{'value': choice[0], 'label': choice[1]} for choice in DashboardWidget.DATA_SOURCES])
