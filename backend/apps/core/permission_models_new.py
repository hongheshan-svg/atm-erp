...
class Permission(BaseModel):
    ...
    parent = models.ForeignKey(
        'self', on_delete=models.CASCADE, null=True, blank=True, related_name='children', verbose_name='父权限'
    )
    code = models.CharField(
        max_length=100, unique=True, verbose_name='权限编码', help_text='唯一标识，如 system:user:create'
    )
    name = models.CharField(max_length=100, verbose_name='权限名称')
    type = models.CharField(max_length=20, choices=PERMISSION_TYPE_CHOICES, verbose_name='权限类型')
    resource = models.CharField(
        max_length=100, blank=True, verbose_name='资源标识', help_text='对应的资源类型，如 user, role'
    )
    field_name = models.CharField(max_length=100, blank=True, verbose_name='字段名称', help_text='字段权限对应的字段名')
    route_path = models.CharField(max_length=200, blank=True, verbose_name='路由路径', help_text='前端路由路径')
    ...
    def clean(self):
        ...
        if self.type == 'field' and not self.field_name:
            raise ValidationError({'field_name': 'Field permissions must have a field_name specified.'})

        # Operation permissions must have resource
        if self.type == 'operation' and not self.resource:
            raise ValidationError({'resource': 'Operation permissions must have a resource specified.'})

        # Prevent self-referencing parent (circular reference)
        if self.parent and self.parent.id == self.id:
            raise ValidationError({'parent': 'A permission cannot be its own parent.'})
...
class RolePermission(models.Model):
    ...
    role = models.ForeignKey(
        'accounts.Role', on_delete=models.CASCADE, related_name='role_permissions', verbose_name='角色'
    )
    permission = models.ForeignKey(
        Permission, on_delete=models.CASCADE, related_name='role_permissions', verbose_name='权限'
    )
...
class DataScope(models.Model):
    ...
    role = models.ForeignKey('accounts.Role', on_delete=models.CASCADE, related_name='data_scopes', verbose_name='角色')
    module = models.CharField(max_length=50, verbose_name='模块', help_text='如 projects, sales, purchase')
    scope_type = models.CharField(max_length=30, choices=SCOPE_TYPE_CHOICES, verbose_name='范围类型')
