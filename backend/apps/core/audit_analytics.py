...
class AuditLogAnalyticsView(APIView):
    ...
    def get(self, request):
        ...
        # 按用户统计（本月）
        by_user = (
            AuditLog.objects.filter(timestamp__date__gte=this_month_start)
        ...
        daily_trend = (
            AuditLog.objects.filter(timestamp__date__gte=thirty_days_ago)
...
class AuditLogSecurityView(APIView):
    ...
    def get(self, request):
        ...
        # 按敏感操作类型统计
        sensitive_by_action = (
            AuditLog.objects.filter(action__in=sensitive_actions, timestamp__date__gte=week_ago)
            .values('action')
            .annotate(count=Count('id'))
            .order_by('-count')
        )
        ...
        # 登录失败统计
        login_stats = {'success': 0, 'failed': 0, 'locked': 0}

        try:
            login_logs = LoginLog.objects.filter(login_time__date__gte=week_ago)
        ...
        # 最近的敏感操作
        recent_sensitive = (
            AuditLog.objects.filter(action__in=sensitive_actions).select_related('user').order_by('-timestamp')[:20]
        )
        ...
        return Response(
            {
                'sensitive_operations': {'count': sensitive_logs, 'by_action': list(sensitive_by_action)},
...
class UserActivityView(APIView):
    ...
    def get(self, request):
        ...
        # 按操作类型
        by_action = (
            user_logs.filter(timestamp__date__gte=this_month_start)
            .values('action')
            .annotate(count=Count('id'))
            .order_by('-count')[:10]
        )
        ...
        thirty_days_ago = today - timedelta(days=30)
        daily_trend = (
            user_logs.filter(timestamp__date__gte=thirty_days_ago)
            .annotate(date=TruncDate('timestamp'))
            .values('date')
            .annotate(count=Count('id'))
            .order_by('date')
