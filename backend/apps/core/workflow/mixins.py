...
class WorkflowEnforcementMixin:
    ...
    def has_active_workflow(self, obj):
        ...
        instance = WorkflowInstance.objects.filter(
            business_type=self.workflow_business_type, business_id=obj.id, status='PENDING', is_deleted=False
        ).first()
    ...
    def check_workflow_allows_direct_action(self, obj, action_name='审批'):
        ...
        if has_workflow:
            return Response(
                {'error': f'该单据正在审批流程中，请通过待办任务进行{action_name}'}, status=status.HTTP_400_BAD_REQUEST
            )
        return None
...
def check_workflow_status(business_type, business_id):
    ...
    """
    instance = WorkflowInstance.objects.filter(
        business_type=business_type, business_id=business_id, status='PENDING', is_deleted=False
    ).first()
