...
class NotificationService:
    ...
    def notify_users(users, title, message, notification_type='INFO', send_email=False):
        ...
        notifications = []
        for user in users:
            notif = NotificationService.create_notification(user, title, message, notification_type)
...
def check_low_stock_and_notify():
    ...
    low_stock_items = Stock.objects.filter(qty_on_hand__lte=F('item__min_stock')).select_related('item', 'warehouse')

    if low_stock_items.exists():
        # Notify procurement team
        procurement_users = User.objects.filter(role__name='Buyer')

        message = f'发现 {low_stock_items.count()} 个库存不足的物料，请及时补货。'

        NotificationService.notify_users(procurement_users, '库存预警', message, 'WARNING', send_email=True)
...
def check_overdue_ar_and_notify():
    ...
    overdue_ar = AccountReceivable.objects.filter(due_date__lt=date.today(), status__in=['PENDING', 'PARTIAL'])

    if overdue_ar.exists():
        # Notify finance team
        finance_users = User.objects.filter(department__name='Finance Department')
        ...
        NotificationService.notify_users(finance_users, '应收账款逾期提醒', message, 'WARNING', send_email=True)
