...
# ---------------------------------------------------------------------------
# G4: 用车行程费 VehicleRequest 从零接入统一核销台账。审计发现归还结算时录入的行程费
# (油费/过路/停车/其他)从未进入应付/核销流程。VehicleRequest.workflow_amount_field
# 为 None(审批锁在用车行为本身而非金额),归还结算(RETURNED)才是"行程费确认"的
# 事实时点,故适配器/信号均以此为触发条件。
# ---------------------------------------------------------------------------
