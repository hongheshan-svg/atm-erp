...
# ---------------------------------------------------------------------------
# G2: 资产维修(AssetMaintenance) + 车辆维护(VehicleMaintenance)自动登记钩子。
# 两者的来源适配器(AssetMaintenancePayableSource/VehicleMaintenancePayableSource,
# write_back 均为 no-op)在前一批已接入,本批补齐 apps/oa/signals.py 的自动登记
# 信号与 backfill_oa_maintenance_payables 回填命令。
# ---------------------------------------------------------------------------
...
class AssetMaintenanceAdapterTest(TestCase):
    """资产维护:AssetMaintenance.status 是处理进度机(PENDING/IN_PROGRESS/COMPLETED/
    CANCELLED),无付款语义,write_back 为 no-op(付款事实由统一台账维护)。"""

    def _make_maintenance(self, code, cost=Decimal('600.00'), status='PENDING'):
        from apps.accounts.models import User
        from apps.oa.asset import Asset, AssetMaintenance
        asset = Asset.objects.create(name=f'测试设备{code}')
        reporter = User.objects.create(username=f'amadp{code}', employee_id=f'AMADP{code}')
        return AssetMaintenance.objects.create(asset=asset, reporter=reporter, cost=cost, status=status)

    def test_register_and_writeback_is_noop(self):
        from apps.finance.payable_adapters import PAYABLE_SOURCES, register_payable
        m = self._make_maintenance('AD1')
        item = register_payable(m, 'asset_maintenance')
        self.assertEqual(item.payee_name, '')
        self.assertIsNone(item.supplier_id)
        ...
        item.amount_paid = Decimal('600.00')
        item.recalc_status()
        item.save()
        PAYABLE_SOURCES['asset_maintenance'].write_back(m, item)
        m.refresh_from_db()
        self.assertEqual(m.status, 'PENDING')  # no-op:进度状态机不受付款影响
...
@override_settings(ELASTICSEARCH_DSL_AUTOSYNC=False)
class VehicleMaintenanceAdapterTest(TestCase):
    """车辆维护:VehicleMaintenance 无 status、无付款字段,write_back 为 no-op。"""

    def _make_vehicle(self, code):
        from apps.oa.vehicle import Vehicle
        return Vehicle.objects.create(plate_number=f'京A{code}', brand='测试', model='测试型号')

    def test_register_and_writeback_is_noop(self):
        from apps.oa.vehicle import VehicleMaintenance
        from apps.finance.payable_adapters import PAYABLE_SOURCES, register_payable
        vehicle = self._make_vehicle('AD1')
        ...
        item.amount_paid = Decimal('450.00')
        item.recalc_status()
        item.save()
        PAYABLE_SOURCES['vehicle_maintenance'].write_back(m, item)  # no-op,不应报错
