...
class Payment(BaseModel):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.payment_no:
            ...
            last_payment = (
                Payment.all_objects.filter(payment_no__startswith=f'PAY{date_str}').order_by('-payment_no').first()
            )
            if last_payment:
...
class PaymentSchedule(BaseModel):
    ...
    def generate_from_sales_order(cls, sales_order):
        ...
        # 软删除旧的付款计划
        cls.objects.filter(sales_order=sales_order, is_deleted=False).update(is_deleted=True, deleted_at=timezone.now())
