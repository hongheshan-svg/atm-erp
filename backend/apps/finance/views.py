...
from .serializers import (
    ...
    PaymentSerializer,
    PurchasePaymentScheduleSerializer,
    SharedExpenseAllocationSerializer,
                ...
                for row_idx, row in enumerate(sheet.iter_rows(min_row=2), start=2):
                    try:
                        ...
                        if existing:
                            ...
                            existing.items.filter(is_deleted=False).update(is_deleted=True, deleted_at=timezone.now())
...
class PaymentScheduleViewSet(PermissionMixin, SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    def record_payment(self, request, pk=None):
        ...
        with transaction.atomic():
            PaymentSchedule.objects.filter(pk=schedule.pk).update(amount_paid=DbF('amount_paid') + amount)
