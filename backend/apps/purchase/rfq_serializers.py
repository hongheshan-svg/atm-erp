...
class RFQLineSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = RFQLine
        fields = [
            'id', 'rfq', 'item', 'item_name', 'item_sku', 'item_unit', 'qty',
            'required_date', 'specifications',
            # 非标自动化增强字段
            'bom_item', 'drawing', 'drawing_name', 'drawing_no', 'drawing_version',
            'technical_specs', 'is_critical', 'is_long_lead', 'is_custom',
            'sample_qty', 'sample_required_date', 'target_price',
            'last_supplier', 'last_supplier_name', 'last_price',
            'attachment_count',
...
class RFQSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = RFQ
        fields = [
            'id', 'rfq_no', 'project', 'project_name', 'project_code', 'request_date',
            'response_deadline', 'status', 'status_display', 'notes',
            # 非标自动化增强字段
            'rfq_type', 'rfq_type_display', 'priority', 'priority_display',
            'template', 'template_name',
            'technical_requirements', 'quality_requirements',
            'packaging_requirements', 'delivery_requirements',
            'attachment_count',
            'lines', 'supplier_rfqs', 'supplier_count', 'line_count',
            'created_at', 'updated_at'
        ]
...
class SupplierQuotationLineSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            # 非标自动化增强字段
            'technical_specs', 'drawing_version', 'drawing_no',
            'sample_qty', 'sample_delivery_days', 'sample_delivery_date', 'sample_unit_price',
            'batch_delivery_date', 'process_capability', 'quality_commitment',
            'inspection_report', 'first_article_inspection',
            # 替代品信息
...
class QuotationScoreSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = [
            ...
            'price_rank', 'delivery_rank', 'quality_rank',
            'score_reliability', 'risk_warnings',
            'created_at', 'updated_at'
        ]
...
class QuotationComparisonSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'weight_template', 'weight_template_display',
            # 权重配置
            'weight_price', 'weight_quality', 'weight_delivery', 'weight_service', 'weight_technical',
...
class CreateComparisonSerializer(serializers.Serializer):
    ...
    comparison_type = serializers.ChoiceField(
        choices=['NORMAL', 'SAMPLE', 'BATCH', 'URGENT', 'CHANGE'],
        default='NORMAL'
    )
    weight_template = serializers.ChoiceField(
        choices=['STANDARD', 'QUALITY_FIRST', 'DELIVERY_FIRST', 'PRICE_FIRST', 'CUSTOM'],
        default='STANDARD'
    )
