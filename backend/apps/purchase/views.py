...
class PurchaseRequestViewSet(
    ...
    def update(self, request, *args, **kwargs):
        ...
        if instance.status not in ['DRAFT', 'REJECTED']:
            return Response({'error': '只有草稿或已拒绝的采购申请可以修改'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def submit(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {'error': f'工作流服务异常，提交失败，请稍后重试: {e}'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def approve(self, request, pk=None):
        ...
        if has_wf and wf_instance:
            pending_task = (
                wf_instance.tasks.filter(status='PENDING', is_deleted=False).order_by('step__step_order').first()
            )
            if pending_task:
                success, msg = WorkflowService.approve_task(
                    pending_task, request.user, request.data.get('comment', ''), skip_assignee_check=True
                )
                if success:
                    pr.refresh_from_db()
        ...
        if has_wf and wf_instance:
            ...
            if pending_task:
                ...
                success, msg = WorkflowService.reject_task(
                    pending_task, request.user, comment, skip_assignee_check=True
                )
                if success:
                    pr.refresh_from_db()
                    return Response(PurchaseRequestSerializer(pr).data)
    ...
    def convert_to_po(self, request, pk=None):
        ...
        if SupplierBlacklist.is_blacklisted(supplier_id):
            return Response({'error': '该供应商已被列入黑名单，不能转换为采购订单'}, status=status.HTTP_400_BAD_REQUEST)
...
class PurchaseOrderViewSet(
    ...
    def update(self, request, *args, **kwargs):
        ...
        instance = self.get_object()
        if instance.status not in ['DRAFT', 'REJECTED']:
            return Response({'error': '只有草稿或已拒绝的订单可以修改'}, status=status.HTTP_400_BAD_REQUEST)
        return super().update(request, *args, **kwargs)
    ...
    def cancel(self, request, pk=None):
        ...
        if has_receipts:
            return Response(
                {'error': '该订单已有确认收货记录，请先退货冲销后再取消'}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def withdraw(self, request, pk=None):
        ...
        if has_receipts:
            return Response(
                {'error': '该订单已有收货单（含草稿），请先删除收货单后再撤回'}, status=status.HTTP_400_BAD_REQUEST
            )
...
class GoodsReceiptViewSet(PermissionMixin, SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    def confirm(self, request, pk=None):
        ...
        if receipt.po.status not in ('CONFIRMED', 'PARTIAL'):
            return Response(
                {'error': '只能对已确认或部分收货状态的采购订单确认收货'}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def return_goods(self, request, pk=None):
        ...
        with transaction.atomic():
            ...
            receipt_lines = {
                rl.po_line_id: rl for rl in receipt.lines.filter(is_deleted=False).exclude(quality_status='FAILED')
            }
            ...
            for item_data in return_items:
                ...
                if receipt_line is None:
                    ...
                    receipt_line = next((rl for rl in receipt_lines.values() if rl.item_id == int(item_id)), None)
                if receipt_line is None:
                    return Response(
...
class PurchaseContractViewSet(
    ...
    def submit(self, request, pk=None):
        ...
        result = self.start_workflow_or_auto_approve(
            contract, request.user, approved_status='APPROVED', submitted_status='PENDING'
        )
