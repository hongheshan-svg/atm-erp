...
def check_delivery_reminders():
    ...
    if overdue_orders.exists():
        ...
        for order in overdue_orders[:10]:
            ...
            message_lines.append(
                f"- {order.order_no} | {order.supplier.name} | "
    ...
    if upcoming_orders.exists():
        ...
        for order in upcoming_orders[:10]:
            ...
            message_lines.append(
                f"- {order.order_no} | {order.supplier.name} | "
                f"¥{order.total_amount:,.2f} | {days_until}天后到货"
            )
    ...
    try:
        ...
        if overdue_items:
            ...
            for item in overdue_items[:5]:
                markdown_content += f"- **{item['order_no']}** | {item['supplier']} | ¥{item['amount']:,.2f} | 逾期{item['days_overdue']}天\n"
            if len(overdue_items) > 5:
                markdown_content += f"  ... 还有 {len(overdue_items) - 5} 笔\n"
        
        if upcoming_items:
            markdown_content += f"\n#### 📅 即将到货 ({len(upcoming_items)}笔)\n"
            for item in upcoming_items[:5]:
                markdown_content += f"- **{item['order_no']}** | {item['supplier']} | ¥{item['amount']:,.2f} | {item['days_until']}天后\n"
