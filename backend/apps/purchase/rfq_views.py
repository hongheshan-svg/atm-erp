...
class RFQTemplateSerializer(serializers.ModelSerializer):
    ...
    def get_default_suppliers_count(self, obj) -> int:
...
class RFQViewSet(PermissionMixin, SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    def create_from_pr(self, request):
        ...
        if not pr_id:
            return Response({'error': '请指定采购申请(purchase_request_id)'}, status=status.HTTP_400_BAD_REQUEST)
...
class QuotationComparisonViewSet(PermissionMixin, SoftDeleteMixin, UserTrackingMixin, viewsets.ModelViewSet):
    ...
    @extend_schema(parameters=[OpenApiParameter('score_id', int, OpenApiParameter.PATH)])
    @action(detail=True, methods=['post'], url_path='update_score/(?P<score_id>[^/.]+)')
...
class ItemPriceHistoryViewSet(PermissionMixin, viewsets.ReadOnlyModelViewSet):
    ...
    @action(detail=False, methods=['get'], url_path='by_item/(?P<item_id>[^/.]+)')
