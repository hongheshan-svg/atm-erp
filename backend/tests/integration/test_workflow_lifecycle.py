...
def test_submit_starts_workflow(api_client_admin, admin_user):
    ...
    assert resp.data['status'] in ('SUBMITTED', 'APPROVED'), f'Unexpected status after submit: {resp.data["status"]}'

    if resp.data.get('workflow_started'):
        assert resp.data['status'] == 'SUBMITTED'
        # WorkflowInstance should exist
        instance = WorkflowInstance.objects.filter(business_type='PURCHASE_REQUEST', business_id=pr_id).first()
...
def test_approve_completes_workflow(api_client_admin, admin_user):
    ...
    WorkflowDefinition.objects.filter(business_type='PURCHASE_REQUEST', is_active=True).update(is_active=False)
