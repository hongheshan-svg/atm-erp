...
PR 按变动选择任务，合并不重复启动全套验证。发版本必须通过发版验证：后端与前端单测全部运行，浏览器按改动模块及关联页面、不含完整业务链，OTA 与安装器只在相关改动时运行；Release 仅复用代码内容完全一致的通过记录，否则先补跑。参数、命令和发布流程见 [CI 与发布操作指南](docs/CI_OPERATIONS.md)。
