...
def run():
    ...
    with sync_playwright() as p:
        ...
        def on_console(msg):
            ...
            elif msg.type == "warning" and (
                "[Vue warn]" in text or "warn" in text.lower()
            ):
        ...
        for i, route in enumerate(ROUTES, 1):
            ...
            parts = []
            if ce_n:
                parts.append(f"{ce_n} err")
            if ne_n:
                parts.append(f"{ne_n} net")
            if vw_n:
                parts.append(f"{vw_n} warn")
    ...
    with open("test_vue_runtime_report.json", "w") as f:
        json.dump(
            {
                "js_errors": list(errs.keys()),
                "network_errors": list(nets.keys()),
                "vue_warnings": list(warns.keys()),
            },
            f,
            ensure_ascii=False,
            indent=2,
