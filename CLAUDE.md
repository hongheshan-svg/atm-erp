...
- 前端在 `frontend/` 下：`npm run lint`、`npm run typecheck`、`npm run build`、`npx vitest run src/modules/modules.spec.ts`（单文件）、`npx playwright test e2e/<file>.spec.ts`。
...
- CI 按影响范围挑选测试：`scripts/ci/impact.py`、`select_suites.py` 计算要跑的模块，发版本强制全量验证，由 `release_gate.py` 核验能否复用同一代码树的全量结果。详见 `docs/CI_OPERATIONS.md`。
