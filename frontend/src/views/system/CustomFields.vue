...
const loadSupportedModels = async () => {
  try {
    const res = await request.get('/api/core/custom-field-definitions/supported_models/')
...
const loadFieldTypes = async () => {
  try {
    const res = await request.get('/api/core/custom-field-definitions/field_types/')
...
const saveField = async () => {
  ...
  try {
    ...
    if (editing.value) {
      await request.put(`/api/core/custom-field-definitions/${editing.value.id}/`, data)
      ElMessage.success('更新成功')
    } else {
      await request.post('/api/core/custom-field-definitions/', data)
...
const toggleVisible = async (field) => {
  try {
    await request.post(`/api/core/custom-field-definitions/${field.id}/toggle_visible/`)
...
const deleteField = async (field) => {
  ...
  try {
    await request.delete(`/api/core/custom-field-definitions/${field.id}/`)
...
const saveSortOrder = async () => {
  ...
  try {
    ...
    await request.post('/api/core/custom-field-definitions/batch_sort/', { items })
