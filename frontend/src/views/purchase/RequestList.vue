...
const currentConvertId = ref<any>(null)
...
const importProject = ref<any>(null)  // 导入时选择的项目
const importError = ref<any>(null)    // 项目号不匹配等错误

// BOM物料筛选相关
const bomProject = ref<any>(null)
const bomItems = ref<any[]>([])
const bomLoading = ref(false)
const bomTableRef = ref<any>(null)
const selectedBomRows = ref<any[]>([])
const bomFilter = reactive<Record<string, any>>({
  hasDrawing: '',
  itemType: '',
  brand: '',
  keyword: ''
...
// 加载项目中未申请的物料
const loadProjectItems = async (projectId: any) => {
  if (!projectId) {
    projectItems.value = []
    return
  }
  ...
  try {
    ...
    // 从BOM中提取物料信息
    projectItems.value = bomItems.map((bom: any) => ({
      id: bom.item,
      ...
      bom_id: bom.id
    })).filter((item: any) => item.id)  // 过滤掉没有item的
  } catch (error: any) {
...
let lastSelectedProject: any = null

// 项目变化时重新加载可选物料
const onProjectChange = async (projectId: any) => {
...
const getFilterTip = () => {
  ...
  if (searchForm.status) {
    ...
    tips.push(`状态: ${(statusMap as Record<string, any>)[searchForm.status]}`)
  }
...
// 获取物料型号/图号
const getItemModel = (itemId: any) => {
  const item = findItemInfo(itemId)
  return item?.item_model || item?.model || item?.drawing_no || '-'
}

// 获取物料类型
const getItemType = (itemId: any) => {
  const item = findItemInfo(itemId)
...
// 计算行税额
const getLineTax = (row: any) => {
  return getLineAmount(row) * (form.tax_rate || 0) / 100
}

// 计算行含税价
const getLineTotalWithTax = (row: any) => {
...
const handleBomSelectionChange = (rows: any) => {
...
const exportBomForQuote = async (bomIds: any, exportName: any) => {
  try {
