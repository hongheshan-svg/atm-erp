  ...
  <div class="preventive-maintenance-list">
    ...
    <el-card>
      <template #header>
        <div class="card-header">
          ...
          <div class="header-actions">
            ...
            <el-button type="primary" v-permission="'sales:preventive_maintenance:create'" @click="showCreateDialog = true">
      ...
      <el-table :data="maintenances" v-loading="loading" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="contract_no" label="服务合同" width="160" show-overflow-tooltip />
        <el-table-column prop="customer_name" label="客户" width="150" show-overflow-tooltip />
        <el-table-column prop="name" label="维护名称" min-width="150" show-overflow-tooltip />
        ...
        <el-table-column label="剩余天数" width="100" align="center">
          <template #default="{ row }">
            <span :class="getDaysClass(daysUntil(row.scheduled_date))">
              {{ formatDaysUntil(daysUntil(row.scheduled_date)) }}
            </span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="showCreateDialog" title="创建预防维护计划" width="600px">
      <el-form :model="maintenanceForm" :rules="maintenanceRules" ref="maintenanceFormRef" label-width="100px">
        <el-form-item label="服务合同" prop="service_contract">
          <el-select v-model="maintenanceForm.service_contract" filterable placeholder="选择服务合同" style="width: 100%">
            <el-option v-for="c in contracts" :key="c.id" :label="c.title" :value="c.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="维护名称" prop="name">
          <el-input v-model="maintenanceForm.name" placeholder="维护计划名称" />
        </el-form-item>
        <el-row :gutter="16">
          <el-col :span="12">
        ...
        <el-form-item label="技术人员">
          <el-select v-model="maintenanceForm.technician" filterable clearable placeholder="选择技术人员" style="width: 100%">
    ...
    <el-drawer v-model="showDetailDrawer" title="维护计划详情" size="50%">
      <template v-if="currentMaintenance">
        <el-descriptions :column="2" border>
          <el-descriptions-item label="服务合同">{{ currentMaintenance.contract_no }}</el-descriptions-item>
          <el-descriptions-item label="客户">{{ currentMaintenance.customer_name }}</el-descriptions-item>
          <el-descriptions-item label="维护名称" :span="2">{{ currentMaintenance.name }}</el-descriptions-item>
          ...
          <el-descriptions-item label="技术人员">{{ currentMaintenance.technician_name || '-' }}</el-descriptions-item>
          <el-descriptions-item label="实际日期">{{ currentMaintenance.actual_date || '-' }}</el-descriptions-item>
          <el-descriptions-item label="维护内容" :span="2">{{ currentMaintenance.description }}</el-descriptions-item>
        </el-descriptions>

        <el-divider v-if="currentMaintenance.checklist && currentMaintenance.checklist.length">检查清单</el-divider>
        <div class="checklist">
          <div v-for="(item, index) in currentMaintenance.checklist" :key="index">
            {{ typeof item === 'string' ? item : item.content }}
          </div>
        </div>

        <el-divider v-if="currentMaintenance.findings || currentMaintenance.actions_taken">完成记录</el-divider>
        <div v-if="currentMaintenance.findings || currentMaintenance.actions_taken" class="completion-notes">
          <p><strong>实际日期:</strong> {{ currentMaintenance.actual_date || '-' }}</p>
          <p><strong>检查发现:</strong> {{ currentMaintenance.findings || '-' }}</p>
          <p><strong>采取措施:</strong> {{ currentMaintenance.actions_taken || '-' }}</p>
          <p><strong>后续建议:</strong> {{ currentMaintenance.next_action || '-' }}</p>
        </div>
      </template>
    </el-drawer>
  </div>
...
const maintenanceRules = {
  service_contract: [{ required: true, message: '请选择服务合同', trigger: 'change' }],
  name: [{ required: true, message: '请输入维护名称', trigger: 'blur' }],
