<template>
  <div class="purchase-payment-schedule-list">
    <!-- 统计卡片 -->
      ...
      <el-col :span="6">
        <el-card shadow="hover" class="summary-card">
          <div class="summary-content">
            ...
            <div class="summary-value text-warning">¥{{ formatMoney(summary.total_remaining) }}</div>
          </div>
        </el-card>
      </el-col>
      <el-col :span="6">
    ...
    <el-card shadow="always">
      ...
      <el-table :data="schedules" stripe border style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="amount_due" label="应付金额" width="120" align="right">
          <template #default="{ row }">
            <span class="text-warning">¥{{ formatMoney(row.amount_due) }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="amount_paid" label="已付金额" width="120" align="right">
          <template #default="{ row }">
            <span class="text-success">¥{{ formatMoney(row.amount_paid) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="付款进度" width="140">
          <template #default="{ row }">
            <el-progress 
        ...
        <el-table-column prop="due_date" label="计划付款日" width="110">
          <template #default="{ row }">
    ...
    <!-- 登记付款对话框 -->
    <el-dialog v-model="paymentDialogVisible" title="登记付款" width="450px">
      <el-form :model="paymentForm" label-width="100px">
        <el-form-item label="采购订单">
          <el-input :value="currentSchedule?.purchase_order_no" disabled />
        </el-form-item>
        <el-form-item label="供应商">
          <el-input :value="currentSchedule?.supplier_name" disabled />
        </el-form-item>
        <el-form-item label="付款节点">
...
const loadSummary = async () => {
  try {
    ...
    const response = await request.get('/finance/purchase-payment-schedules/summary/', { params })
...
const submitPayment = async () => {
  ...
  try {
    await request.post(`/finance/purchase-payment-schedules/${currentSchedule.value.id}/record_payment/`, {
    ...
    ElMessage.error('付款登记失败: ' + (error.message || '未知错误'))
  } finally {
...
<style scoped>
.purchase-payment-schedule-list {
  padding: 16px;
}
