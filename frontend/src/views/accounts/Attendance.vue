...
const fetchTodayStatus = async () => {
  try {
    const { data } = await request.get('/accounts/attendance-records/today/')
...
const fetchRecords = async () => {
  loading.value = true
  try {
    const { data } = await request.get('/accounts/attendance-records/my_records/', {
...
const fetchLeaveRequests = async () => {
  try {
    const { data } = await request.get('/accounts/leave-requests/my_requests/')
...
const fetchOvertimeRequests = async () => {
  try {
    const { data } = await request.get('/accounts/overtime-requests/my_requests/')
...
const fetchSummary = async () => {
  try {
    const { data } = await request.get('/accounts/attendance-records/monthly_summary/', {
...
const fetchLeaveTypes = async () => {
  try {
    const { data } = await request.get('/accounts/leave-requests/leave_types/')
...
const handleCheckIn = async () => {
  try {
    const { data } = await request.post('/accounts/attendance-records/check_in/', {
      location: '办公室'
    })
    todayStatus.value = data
...
const handleCheckOut = async () => {
  try {
    const { data } = await request.post('/accounts/attendance-records/check_out/', {
      location: '办公室'
    })
    todayStatus.value = data
...
const submitLeaveForm = async () => {
  ...
  try {
    await request.post('/accounts/leave-requests/', leaveForm)
...
const submitOvertimeForm = async () => {
  ...
  submitLoading.value = true
  try {
    await request.post('/accounts/overtime-requests/', overtimeForm)
    ElMessage.success('申请已保存')
