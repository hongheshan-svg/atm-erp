...
const feedback = ref('')
let generation = 0
watch([supplier, month], () => { generation++; data.value = null; command.value = null; busy.value = false; feedback.value = '' }, { flush: 'sync' })
...
async function load() {
  ...
  try {
    const report = await read('/business/reconciliations/supplier-monthly/', { supplier: supplier.value, month: month.value })
    if (current === generation) data.value = report
  }
  catch (e) { if (current === generation) error.value = message(e) } finally { if (current === generation) busy.value = false }
}
function confirm() {
  const report = data.value!
  if (!report || String(report.supplier) !== String(supplier.value) || report.month !== month.value) return
  command.value = { title: '确认供应商月度对账', path: '/business/reconciliations/supplier-monthly/',
    subject: `${suppliers.value.find(s => s.id === report.supplier)?.name || report.supplier} · ${report.month}`,
...
<template>
  <section class="panel" aria-label="供应商月度对账">
    ...
    <template v-if="data">
      <p>当前报告：{{ suppliers.find(s => s.id === data!.supplier)?.name }} · {{ data.month }}</p>
