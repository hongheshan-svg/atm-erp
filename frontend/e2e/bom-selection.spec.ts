...
test('采购按品牌单元类别组合多选，跨页保留勾选且只生成所选BOM行', async ({ page }, info) => {
  ...
  const items = await importRows(page, '物料', '物料编码,名称,规格,单位,品牌,物料类别\n' + Array.from({ length: 13 }, (_, i) => `${tag}-${i},测试件${tag}-${i},规格,件,${i < 12 ? '品牌A' : '品牌B'},${i < 12 ? 'standard' : 'custom'}`).join('\n'))
  ...
  await bom.locator('input[type=file]').setInputFiles({ name: 'bom.csv', mimeType: 'text/csv', buffer: Buffer.from('物料编码,单元,需求数量,变更说明\n' + Array.from({ length: 13 }, (_, i) => `${tag}-${i},${i < 12 ? '上料单元' : '检测单元'},1,初版`).join('\n')) })
